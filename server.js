...
app.patch('/api/boissons/:id', exigerRole('admin', 'gerant'), async (req, res) => {
  const {
    stock_bouteilles, nom, categorie, taille, unites_par_carton,
    prix_achat_carton, prix_achat_unite, prix_vente_bouteille,
    prix_vente_verre_petit, prix_vente_verre_grand, seuil
  } = req.body;
  try {
    const champs = [];
    const valeurs = [];
    let i = 1;
    if (stock_bouteilles !== undefined) { champs.push(`stock_bouteilles = $${i++}`); valeurs.push(Math.max(0, stock_bouteilles)); }
    if (nom !== undefined) { champs.push(`nom = $${i++}`); valeurs.push(nom); }
    if (categorie !== undefined) { champs.push(`categorie = $${i++}`); valeurs.push(categorie); }
    if (taille !== undefined) { champs.push(`taille = $${i++}`); valeurs.push(taille || null); }
    if (unites_par_carton !== undefined) { champs.push(`unites_par_carton = $${i++}`); valeurs.push(unites_par_carton || null); }
    if (prix_achat_carton !== undefined) { champs.push(`prix_achat_carton = $${i++}`); valeurs.push(prix_achat_carton || null); }
    if (prix_achat_unite !== undefined) { champs.push(`prix_achat_unite = $${i++}`); valeurs.push(prix_achat_unite || null); }
    if (prix_vente_bouteille !== undefined) { champs.push(`prix_vente_bouteille = $${i++}`); valeurs.push(prix_vente_bouteille || 0); }
    if (prix_vente_verre_petit !== undefined) { champs.push(`prix_vente_verre_petit = $${i++}`); valeurs.push(prix_vente_verre_petit || null); }
    if (prix_vente_verre_grand !== undefined) { champs.push(`prix_vente_verre_grand = $${i++}`); valeurs.push(prix_vente_verre_grand || null); }
    if (seuil !== undefined) { champs.push(`seuil = $${i++}`); valeurs.push(seuil); }
    if (champs.length === 0) return res.status(400).json({ erreur: 'Aucune modification fournie.' });
    champs.push(`modifie_par = $${i++}`); valeurs.push(req.utilisateur.prenom);
    champs.push('modifie_le = now()');
    valeurs.push(req.params.id);

    const { rows } = await pool.query(
      `UPDATE boissons SET ${champs.join(', ')} WHERE id = $${i} RETURNING *`,
      valeurs
    );
