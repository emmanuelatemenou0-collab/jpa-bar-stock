...
// Retour partiel ou total : le client ramène une partie de ce qui a été pris.
// On défalque la quantité de la commande (et donc du total et du bilan de la serveuse),
// et on remet en stock si c'était une vente à la bouteille.
app.post('/api/commandes/:id/retour', async (req, res) => {
  const { quantite } = req.body;
  const qteRetour = Number(quantite);
  if (!qteRetour || qteRetour <= 0) return res.status(400).json({ erreur: 'Quantité de retour invalide.' });
  try {
    const { rows } = await pool.query('SELECT * FROM commandes WHERE id = $1', [req.params.id]);
    if (rows.length === 0) return res.status(404).json({ erreur: 'Commande introuvable.' });
    const c = rows[0];

    // Une serveuse ne peut faire un retour que sur l'une de ses propres commandes
    if (req.utilisateur.role === 'serveuse' && c.serveuse_id !== req.utilisateur.serveuse_id) {
      return res.status(403).json({ erreur: 'Vous ne pouvez modifier que vos propres commandes.' });
    }

    if (qteRetour > Number(c.quantite)) {
      return res.status(400).json({ erreur: 'La quantité retournée dépasse la quantité vendue.' });
    }

    const nouvelleQuantite = Number(c.quantite) - qteRetour;

    if (c.type_vente === 'bouteille' && c.boisson_id) {
      await pool.query('UPDATE boissons SET stock_bouteilles = stock_bouteilles + $1 WHERE id = $2', [qteRetour, c.boisson_id]);
    }

    if (nouvelleQuantite <= 0) {
      await pool.query('DELETE FROM commandes WHERE id = $1', [req.params.id]);
      return res.json({ supprimee: true });
    }

    const nouveauTotal = Number(c.prix_unitaire) * nouvelleQuantite;
    const { rows: maj } = await pool.query(
      `UPDATE commandes SET quantite = $1, total = $2, modifie_par = $3 WHERE id = $4 RETURNING *`,
      [nouvelleQuantite, nouveauTotal, req.utilisateur.prenom, req.params.id]
    );
    res.json(maj[0]);
  } catch (e) { erreurServeur(res, e); }
});
